...
const validateImageCreate = [
  check("url").exists({ checkFalsy: true }).withMessage("Must provide a url"),
...
// Create Group
router.post("/", validateGroupCreate, async (req, res, next) => {
  const { user } = req;
  const currentId = user.id;
...
router.post("/:groupId/images", validateImageCreate, async (req, res, next) => {
  const { user } = req;
  const currentId = user.id;
  const group = await Group.findByPk(req.params.groupId);
  if (!group) return next(noGroupErr);
