...
function GetSingleGroup() {
  ...
  let previewImage = group?.GroupImages.filter((e) => (e.preview = true))[0]
    ?.url;
  ...
  const numMembers = group?.Members
        ...
        <div className="groupInfo">
          <div className="group-title-card">
            ...
            <div className="group-specifics">
              ...
              <div className="specifics-lines">
                ...
                <p>
                  {`${numMembers} ${memberString}`} · {privateString} group
                </p>
              </div>
              <div className="specifics-lines">
          ...
          {group?.Members[user?.id] && (
            <div className="group-edit-buttons">
              ...
              {userMember?.status === "cohost" && (
                <PendingMembershipsModal pending={group?.PendingMembers} />
              )}
            </div>
          )}
          <div className="group-edit-buttons">
            {!userMember && !pendingMember && user && (
              <button onClick={requestMembershipButton}>
                Request Membership
              </button>
            )}
            {pendingMember && (
